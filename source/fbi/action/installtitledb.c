...
void action_install_titledb(linked_list* items, list_item* selected, bool cia) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Non è stato possibile allocare i dati da TitleDB.");
    ...
    if(data->cia) {
        snprintf(urls, sizeof(urls), "http://3ds.titledb.ga:7080/v1/cia/%lu/download", info->cia.id);
    } else {
        ...
        urlsPos += snprintf(urls + urlsPos, sizeof(urls) - urlsPos, "http://3ds.titledb.ga:7080/v1/tdsx/%lu/download\n", info->tdsx.id);
        ...
        if(info->tdsx.smdh.exists) {
            ...
            snprintf(urls + urlsPos, sizeof(urls) - urlsPos, "http://3ds.titledb.ga:7080/v1/smdh/%lu/download\n", info->tdsx.smdh.id);
            snprintf(paths + pathsPos, sizeof(paths) - pathsPos, "%s\n", filePath);
        }
    }

    action_install_url("Vuoi installare il file selezionato da TitleDB?", urls, paths, data, action_install_titledb_finished_url, action_install_titledb_finished_all, action_install_titledb_draw_top);
}