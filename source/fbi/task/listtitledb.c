...
static void task_populate_titledb_thread(void* arg) {
    ...
    if(text != NULL) {
        u32 textSize = 0;
        if(R_SUCCEEDED(res = task_populate_titledb_download(&textSize, text, maxTextSize, "http://api.titledb.ga:7080/v0/"))) {
    ...
    if(R_SUCCEEDED(res)) {
        ...
        while(linked_list_iter_has_next(&iter)) {
            ...
            if(png != NULL) {
                char pngUrl[128];
                snprintf(pngUrl, sizeof(pngUrl), "http://api.titledb.ga:7080/v0/images/%016llX.png", titledbInfo->titleId);
