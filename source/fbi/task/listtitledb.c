...
static void task_populate_titledb_thread(void* arg) {
    ...
    json_t* root = NULL;
    if(R_SUCCEEDED(res = http_download_json("http://api.titledb.ga:7080/v1/entry?nested=true"
    ...
    if(R_SUCCEEDED(res)) {
        ...
        while(linked_list_iter_has_next(&iter)) {
            ...
            if(titledbInfo->cia.exists) {
                snprintf(url, sizeof(url), "http://3ds.titledb.ga:7080/v1/cia/%lu/icon_l.bin", titledbInfo->cia.id);
            } else if(titledbInfo->tdsx.exists && titledbInfo->tdsx.smdh.exists) {
                snprintf(url, sizeof(url), "http://3ds.titledb.ga:7080/v1/smdh/%lu/icon_l.bin", titledbInfo->tdsx.smdh.id);
            } else {
                continue;
            }
