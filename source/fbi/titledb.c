...
static void titledb_action_open(linked_list* items, list_item* selected, bool cia) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Non è stato possibile allocare i dati delle azioni TitleDB.");
    ...
    list_display("Azioni TitleDB", "A: Seleziona, B: Indietro", data, titledb_action_update, titledb_action_draw_top);
}
...
static void titledb_entry_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Non è stato possibile allocare i dati dell'entrata di TitleDB.");
    ...
    list_display("Entrata TitleDB", "A: Seleziona, B: Indietro", data, titledb_entry_update, titledb_entry_draw_top);
}
...
static void titledb_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        titledb_options_add_entry(items, "Mostra CIA", &listData->showCIAs);
        titledb_options_add_entry(items, "Mostra 3DSX", &listData->show3DSXs);
        titledb_options_add_entry(items, "Ordina per nome", &listData->sortByName);
        titledb_options_add_entry(items, "Ordina per data di aggiornamento", &listData->sortByUpdate);
        titledb_options_add_entry(items, "Ordina per stato di installazione", &listData->sortByStatus);
    }
}
...
static void titledb_options_open(titledb_data* parent, linked_list* items) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Non è stato possibile allocare i dati delle opzioni di TitleDB.");
    ...
    list_display("Opzioni", "A: Imposta, B: Indietro", data, titledb_options_update, NULL);
}
...
static void titledb_draw_top(ui_view* view, void* data, float x1, float y1, float x2, float y2, list_item* selected) {
    ...
    if(!listData->populateData.itemsListed) {
        static const char* text = "Sto scaricando la lista dei giochi...\nNOTA: Se torni indietro adesso ci potrebbero volere 15 secondi.";
...
static void titledb_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Non è stato possibile ottenere creare una lista.");
        }
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "Non è stato possibile ottenere la lista dei giochi.");
...
void titledb_open() {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Non è stato possibile allocare i dati di TitleDB.");
    ...
    list_display("https://discord.gg/ptQg9kM", "A: Seleziona, B: Indietro, X: Ricarica, Y: Aggiorna tutto, Select: Opzioni", data, titledb_update, titledb_draw_top);
}