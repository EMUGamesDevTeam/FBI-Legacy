...
static void update_check_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    json_t* json = NULL;
    if(R_SUCCEEDED(res = http_download_json("https://api.github.com/repos/EMUGamesDevTeam/FBI/releases/latest", &json, 16 * 1024))) {
    ...
    if(hasUpdate) {
        action_install_url("Vuoi aggiornare FBI all'ultima versione?", updateURL, fs_get_3dsx_path(), NULL, NULL, NULL, NULL);
    } else {
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Impossibile controllare gli aggiornamenti.");
        } else {
            prompt_display_notify("Fatto.", "Stai usando l'ultima versione.", COLOR_TEXT, NULL, NULL, NULL);
        }
    }
}

void update_open() {
    info_display("Controllo aggiornamenti in corso...", "", false, NULL, update_check_update, NULL);
}
